...
use12 <- read.csv("..\\saved\\GBYTuse12table.csv", header = TRUE)
use12
nscen <- length(use12$scenario)
...
spr.f <- function(F.start){
  abs(rose::calc_ssb_per_r(nAge = orig$nAge, ssbwaa = orig$ssbwaa, maturity = orig$maturity, maa = orig$maa, fmult = F.start, selx = orig$selx, spawntime = orig$spawntime) / ssbpr0 - target.spr)
}
...
ssbprtarget.orig <- rose::calc_ssb_per_r(orig$nAge, orig$ssbwaa, orig$maturity, orig$maa, Ftarget.orig, orig$selx, orig$spawntime)
...
for (i in 1:nscen){
  asap <- dget(paste0("..\\saved\\GBYTscen", scennum[i], ".rdat"))
  ...
  projcatch[i, ] <- project_short_term(naa, mya$maa, Ftarget[i], mya$selx, mya$cwaa, mynpy, naa[1]) / use12$cmult[i]
  
}

# save data
scen.status.df <- data.frame(Scenario = rep(paste(scennum, use12$scenario, sep="_"), 3),
...
write.csv(status.df, file = "..\\saved\\GBYTstatus.csv", row.names = FALSE)
...
scen.c.df <- data.frame(Scenario = rep(paste(scennum, use12$scenario, sep="_"), 3),
...
write.csv(c.df, file = "..\\saved\\GBYTcatch_advice.csv", row.names = FALSE)
